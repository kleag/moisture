...
def load_environment_file(envfile, key_length=64):
    print(f"load_environment_file {envfile}")
    config = None
    lock = FileLock(os.path.abspath(envfile) + ".lock")

    with lock:
        if not os.path.exists(envfile):
            print(f"load_environment_file {envfile} does not exist")
        ...
        for key, value in config.items('django'):
            #print(f"load_environment_file {key}/{value}")
            ...
            if envname not in os.environ:  # Don't replace existing defined variables
                print(f"load_environment_file set environment variable {envname}={value}")
